...
- `Simulation<TState, TParameters>` is a required base contract plus optional capability method groups. Simulations should only implement playback, deterministic step, parameters, or presets when they declare and support those capabilities.
...
- `TimeSeriesGraph` in `src/ui/graphs/TimeSeriesGraph.tsx` provides the initial dependency-free SVG graph capability for Prototype A telemetry. It uses explicit data points and deterministic path scaling, leaving graph sample generation in the model/controller layer.
