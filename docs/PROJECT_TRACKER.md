...
| SS-305 | Implement graph/telemetry behavior            | DONE   | PENDING | Graph samples are generated from model/controller state; fast QA, build, and desktop browser checks pass.                                     |
| SS-306 | Implement presentation mode behavior          | DONE   | PENDING | Presentation mode provides a focused 16:9 teaching stage with compact playback, graph, and readings; fast QA, build, and browser checks pass. |
| SS-307 | Verify resource cleanup                       | DONE   | PENDING | Scene view unmount test verifies renderer destroy and resize observer disconnect; fast QA and build pass.                                     |
